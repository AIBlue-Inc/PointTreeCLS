...
from data_nifosfinal_gomsol import NifosTreeFinalGomsol as NifosTree
...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=54, help='batch size in training')
...
def main():
    ...
    if not os.path.isfile(os.path.join(args.checkpoint, "last_checkpoint.pth")):
        ...
        # Load the state dict, set strict=False to ignore missing keys.
    ...
    train_loader = DataLoader(NifosTree(partition='train', num_points=args.num_points), num_workers=args.workers,
                              batch_size=args.batch_size, shuffle=True, drop_last=True)
    test_loader = DataLoader(NifosTree(partition='valid', num_points=args.num_points), num_workers=args.workers,
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        train_out, step = train(net, train_loader, optimizer, criterion, device, global_step)  # {"loss", "acc", "acc_avg", "time"}
...
def train(net, trainloader, optimizer, criterion, device, global_step):
    net.train()
    train_loss = 0
    correct = 0
    total = 0
    ...
    for batch_idx, (data, label) in enumerate(trainloader):
        ...
        wandb.log({"train_loss": train_loss / (batch_idx + 1), "train_acc": 100. * correct / total}, step=global_step)
        global_step += 1
