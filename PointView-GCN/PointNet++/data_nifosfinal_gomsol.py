...
class_names = ['Densi', 'gul', 'Koraiensis', 'Larix', 'obtusa', 'sang', 'sin', 'thunbergii']
...
class NifosTreeFinalGomsol(Dataset):
...
if __name__ == '__main__':
    train = NifosTreeFinalGomsol(1024)
    test = NifosTreeFinalGomsol(1024, 'test')
    # for data, label in train:
    #     print(data.shape)
    #     print(label.shape)
    from torch.utils.data import DataLoader

    train_loader = DataLoader(NifosTreeFinalGomsol(partition='train', num_points=1024), num_workers=4,
                              batch_size=32, shuffle=True, drop_last=True)
    for batch_idx, (data, label) in enumerate(train_loader):
        print(f"batch_idx: {batch_idx}  | data shape: {data.shape} | ;label shape: {label.shape}")

    train_set = NifosTreeFinalGomsol(partition='train', num_points=1024)
    test_set = NifosTreeFinalGomsol(partition='valid', num_points=1024)
